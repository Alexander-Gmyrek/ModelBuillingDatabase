// apiFunctions.js
const config = {
    MY_BASE_URL: window.BILLING_API_BASE_URL || "http://localhost:3000"
};
...
async function terminateEmployee(employeeId, terminationJson) {
    return await apiRequest(`/employee/${employeeId}/terminate`, "PATCH", terminationJson);
}
...
async function searchActiveEmployeePlans(employeeID) {
    return await apiRequest(`/employeeplan/${encodeURIComponent(employeeID)}/active`);
}
...
async function endPlan(planId) {
    return await apiRequest(`/plan/${planId}/end`, "PATCH", {});
}
...
async function saveEmployerPlanSetup(employerId, planSetupJson) {
    return await apiRequest(`/employer/${employerId}/plan_setup`, "PATCH", planSetupJson);
}
